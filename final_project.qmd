...
These missing variables are however in the minority in the final data base and will not affect the results for our analysis.
...
In order to compare the share of museums and the share of visitors a bit better, here is a graph showing them side by side, where we can see that the comparison between share of museums and share of visitors is outstandingly noticeable for Île-de-France.
...
# deleting the rows that weren't common to both data base + deleting the overseas regions
regions <- filter(regions, nom != "Mayotte", nom != "Guadeloupe", nom != "Guyane", nom != "La Réunion", nom != "Martinique")
vitality <- filter(vitality, regions != "ST PIERRE ET MIQUELON", regions != "GUADELOUPE", regions != "GUYANE", regions != "LA REUNION", regions != "MARTINIQUE")
# creating an index (to facilitate the joining)
id <- as.character(1:13)
...
plot + labs(title = "Cultural vitality in France", subtitle = "Number of museums in each french region") + theme(legend.key.size = unit(0.3, "cm"))
```
...
data |>
  ...
  labs(title = "Comparison of the number of museums in Paris and \nother location ",
       x = "Location of the museum",
       y = "Number of museums") +
...
data |> filter(!is.na(total)) |>
  ...
  geom_bar(stat = "identity", fill = "darkgreen") +
  labs(title = "Frequentation of museums located in Paris and \nother location",
       x = "Location of the museum",
       y = "Cumulative frequentation") +
  scale_y_continuous(labels = scales::comma) +
  scale_x_discrete(labels = c("other location", "located in Paris"))
...
ggplot(category_count, aes(x = category, y = total_museums, fill = category)) +
  ...
  labs(title = "Distribution of museum theme (considering \ncategories individually)",
       x = "Categories",
       y = "Number of museums") +
...
ggplot(region_count, aes(x = regions, y = total_museums, fill = category)) +
  geom_col(position = "dodge") +
  labs(title = "Distribution of museum theme at the \nregional level",
       x = "Regions",
       y = "Number of museums") +
  theme(axis.title = element_text(size = 13,
                                  color = "purple",
...
data |> filter(!is.na(total)) |> filter(!is.na(category)) |>
  ...
  geom_bar(stat="identity", fill = "darkgreen") +
  labs(title = "Frequentation at the national level \ndepending on types",
       x = "Categories",
       y = "Number of museums") +
  coord_flip() +
  scale_y_continuous(labels = scales::comma)
...
ggplot(freq_count, aes(x = category, y = freq_category, fill = category)) +
  geom_bar(stat = "identity") +
  labs(title = "Frequentation of museums (considering \ncategories individually)",
       x = "Categories",
...
```{r}
plot(data$city, data$total, main = "Link between being located in a big city \nand museums attendance", xlab = "Located in one of the 10 biggest cities", ylab = "Total visits in a year", xaxt = "n", ylim = c(0, 700000))
...
```{r}
plot(data$city, data$total, main = "Link between being located in Paris \nand museums attendance", xlab = "Located in Paris", ylab = "Total visits", xaxt = "n",ylim = c(0, 700000))
...
par(mfrow = c(1,3))
# plot 1
plot(data$art, data$total, main = "Impact of being an \nart museum \non its attendance", xlab = "Being an art museum", ylab = "Total visits", xaxt = "n", ylim = c(0, 500000))
abline(third_regression, col = "darkgreen")
axis(1, at = 0:1)
# plot 2
plot(data$history, data$total, main = "Impact of being a \nhistory museum \non its attendance", xlab = "Being a history museum", ylab = "Total visits", xaxt = "n",ylim = c(0, 500000))
abline(fourth_regression, col = "pink")
axis(1, at = 0:1)
# plot 3
plot(data$science, data$total, main = "Impact of being a \nscience museum \non its attendance", xlab = "Being a science museum", ylab = "Total visits", xaxt = "n",ylim = c(0, 500000))
