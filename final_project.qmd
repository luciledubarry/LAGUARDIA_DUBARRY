...
We indicate below the number of NA in the key variables of our analysis: - in the attendance data base: we do not have the attendance record of`r NA_total` museums, which will not be counted when analyzing the frequentation records. - in the museum data base: `r NA_dompal` museums do not indicate their themes, then we will not take these museums into account when studying the museum themes. These missing variables are however in the minority in the final data base (on `r nrow(data)` museums) and will not affect the results for our analysis.
...
From this regression, we can say that the average number of visitors per year for a museum not located in one of the 10 biggest cities is estimated to be 34856. For a museum located in one of those cities, on the other hand, it is estimated to be 34856 + 260451, which is equal to: `r 34856 + 260451` visitors.
...
From this regression, we can say that the average number of visitors per year for a museum not located in Paris is estimated to be 39521. For a museum located in Paris, on the other hand, it is estimated to be 39521 + 501212, which is equal to: `r 39521 + 501212` visitors.
...
### The impact of a museum's theme on attendance
...
par(mfrow = c(1,3))
# plot 1
plot(data$art, data$total, main = "Link between being an art museum and its attendance", xlab = "Being an art museum", ylab = "Total visits", xaxt = "n", ylim = c(0, 500000))
abline(third_regression, col = "darkgreen")
axis(1, at = 0:1)
# plot 2
plot(data$history, data$total, main = "Link between being a history museum and its attendance", xlab = "Being a history museum", ylab = "Total visits", xaxt = "n",ylim = c(0, 500000))
abline(fourth_regression, col = "pink")
axis(1, at = 0:1)
# plot 3
plot(data$science, data$total, main = "Link between being a science museum and its attendance", xlab = "Being a science museum", ylab = "Total visits", xaxt = "n",ylim = c(0, 500000))
...
Another question that could be asked is the impact of the museum's theme on the pre-existing relation between location and attendance. Is the impact of being located in Paris stronger if the museum is an Art museum ? Let's look into it.
