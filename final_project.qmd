...
```{r}
missing_values <- rowSums(is.na(data))
museum_with_NA<- sum(missing_values > 0)
```

`r museum_with_NA` museums in the final data base has at least one missing value. However, most of these missing values do not concern the variables of interest on which our analysis is based. 
