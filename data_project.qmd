...
The freq data set only includes Muséofile references starting from 2019. The years following 2019 are highly impacted by Covid crisis. We don't take into account these years in our study, as they are not representative of the museum usual frequentation. 
...
We join the data bases with reference to the Muséofile id number. On the Museofile data database it is the variable named REF and on the freq database it is the variable named ID_MUSEOFILE.
...
Because of a lack of data, a year-by-year comparison of attendance is unfeasible, as the dataset only includes Muséofile references starting from 2019.
